  ...
  return (
    <div className={styles.GridAlignCards}>
      {InfoRowListData.ImageSinglePropsList.map((Imageprop) => (
        <ImageCard key={Imageprop.title} image={Imageprop.image} category={Imageprop.category} title={Imageprop.title} description={Imageprop.description} link={Imageprop.link} priority={true} />
      ))}
    </div>
  );
};
